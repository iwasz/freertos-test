// Code taken from https://gist.github.com/JayKickliter/a3afab9af3908e42faf9f81b442bc263
...
static inline bool lptim_should_load_arr (uint32_t cnt_current, uint32_t arr_current, uint32_t arr_new);

static inline uint32_t lptim_calculate_arr_for_next_n_ticks (uint32_t cnt, uint32_t n_ticks);

static inline uint32_t lptim_calculate_tick_from_cnt (uint32_t cnt);

static inline void lptim_load_arr_from_init (LPTIM_HandleTypeDef *t, uint32_t arr);
static inline void lptim_load_arr (LPTIM_HandleTypeDef *t, uint32_t arr);
...
static inline uint32_t lptim_calculate_arr_for_next_n_ticks (uint32_t cnt, uint32_t n_ticks)
{
        ...
        const uint32_t rem_cnts = LPTIM1_CNTS_PER_TICK - (cnt % LPTIM1_CNTS_PER_TICK);
 ...
 */
static inline uint32_t lptim_calculate_tick_from_cnt (uint32_t cnt) { return cnt / LPTIM1_CNTS_PER_TICK; }

static inline bool lptim_arr_is_stable (LPTIM_HandleTypeDef *t) { return __HAL_LPTIM_GET_FLAG (t, LPTIM_FLAG_ARROK); }
 ...
 */
static inline uint32_t lptim_read_arr (LPTIM_HandleTypeDef *t) { return t->Instance->ARR; }
...
void vPortSuppressTicksAndSleep (TickType_t expected_idle_ticks)
{
        ...
        const uint32_t pre_sleep_tick = lptim_calculate_tick_from_cnt (pre_sleep_cnt);
...
//        }
        ...
        const uint32_t arr_for_sleep = lptim_calculate_arr_for_next_n_ticks (pre_sleep_cnt, expected_idle_ticks);
        ...
        if (eTaskConfirmSleepModeStatus () == eAbortSleep) {
                ...
                bool woke_by_lptim_arrm = __HAL_LPTIM_GET_FLAG (&lptim_handle, LPTIM_FLAG_ARRM);
...
//                        dbg_util_event_log (dbg_lptim_complete_unstable_arr);
                         ...
                         * LPTIM1 CNT register. */
                        uint32_t const post_sleep_tick = lptim_calculate_tick_from_cnt (post_sleep_arr);
                        ...
                        uint32_t const next_arr = lptim_calculate_arr_for_next_n_ticks (0, 1);
                        ...
                        uint32_t const post_sleep_tick = lptim_calculate_tick_from_cnt (post_sleep_cnt);
                        completed_ticks = post_sleep_tick - pre_sleep_tick;
                        assert (completed_ticks <= post_sleep_tick);

                        /* Calculate ARR for the next tick boundary */
                        uint32_t const next_arr_0 = lptim_calculate_arr_for_next_n_ticks (post_sleep_cnt, 1);
                        /* In case we're too close to the next tick boundary, calculate ARR
                         * for two tick boundaries from now */
                        uint32_t const next_arr_1 = lptim_calculate_arr_for_next_n_ticks (post_sleep_cnt, 2);

                        if (lptim_should_load_arr (post_sleep_cnt, arr_for_sleep, next_arr_0)) {
                                /* We can load ARR for the next tick boundary */
                                assert (next_arr_0 <= arr_for_sleep);
                                lptim_load_arr (&lptim_handle, next_arr_0);
                        }
                        else if (lptim_should_load_arr (post_sleep_cnt, arr_for_sleep, next_arr_1)) {
                                ...
                                lptim_load_arr (&lptim_handle, lptim_calculate_arr_for_next_n_ticks (0, 1));
                        }
                        else {
