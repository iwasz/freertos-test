...
#error Please edit FreeRTOSConfig.h to define configUSE_TICKLESS_IDLE as 2 *or* exclude this file from this build config.
#else
...
void vPortSuppressTicksAndSleep (TickType_t xExpectedIdleTime)
{
        ...
        if (eTaskConfirmSleepModeStatus () == eAbortSleep)
                ...
                if (idealCmp == expectedEndCmp) {
                        ...
                        uint32_t currCount;
                        do
                                currCount = LPTIM1->CNT;
                        while (currCount != LPTIM1->CNT);
...
void LPTIM1_IRQHandler (void)
{
        ...
        if (isr & LPTIM_ISR_CMPM) {
                ...
                uint32_t countValue;
                do
                        countValue = LPTIM1->CNT;
                while (countValue != LPTIM1->CNT);
